...
do_check_protect_pse36:
 ...
 do_fault:
    ...
    /* Notify the OS kernel it is a Virtual Descriptor Fault
       (VDF) via the error code. The OS cannot double fault when
       handling VDF
     */
    if(is_VDF) {
        env->error_code |= PG_ERROR_VDF_MASK;
        PA_log("[VDF] do_fault: 0x%" PRIx64, env->cr[2]);
    } else if(Reg_kapp) {
        PA_log("[PF] do_fault: 0x%" PRIx64, env->cr[2]);
    }
