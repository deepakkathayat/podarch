...
/* This is for handling VDF in PodArch */
...
#define CPUID_PODARCH (1U << 10) /* PodArch */
...
#include "exec/pod-common.h"

extern target_ulong kcpu;
extern cr3_table_struct *cr3_table;
extern ipact_struct *ipact;
...
#ifdef CONFIG_PODARCH
            /* Update the Reg_Kapp depending on the CPL and Process */
            if (cpl == 3) {
                cr3_table_struct *temp;
                HASH_FIND(cr3_table_hash, cr3_table, &env->cr[3], sizeof(target_ulong), temp);
                if (temp) {
                    Reg_kapp = temp->kapp;
                    PA_log("Inside cpl == 3 && Reg_kapp == %" PRIu64, Reg_kapp);
                }
                else {
                    Reg_kapp = 0;
                }
