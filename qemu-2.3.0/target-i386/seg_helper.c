...
#define POD_DELVA(num, start, end)                                                            \
({                                                                                            \
            /* Flush the corresponding I-PACT entries */                                      \
            target_ulong addr, rem_size, size = 0;                                            \
            rem_size = end - start;                                                           \
            while(start + size < end) {                                                       \
                addr = start + size;                                                          \
                                                                                              \
                /* If already emulated state just ignore re-computation */                    \
                if (addr < temp->qemu_delva_state) {                                          \
                    if (PG_SIZE > rem_size) {                                                 \
                        size +=  rem_size;                                                    \
                        rem_size = 0;                                                         \
                    } else {                                                                  \
                ...
                target_ulong tlb_addr = env->tlb_table[MMU_USER_IDX][index].addr_write;       \
                                                                                              \
                /* If a miss, handle again */                                                 \
                if((addr & TARGET_PAGE_MASK) !=                                               \
                    (tlb_addr & (TARGET_PAGE_MASK | TLB_INVALID_MASK))) {                     \
                    handle_tlb_miss_again(env, addr, NORMAL_PF, (uintptr_t) NULL, 0);         \
                }                                                                             \
                                                                                              \
                target_ulong PPN =                                                            \
                    (addr & TARGET_PAGE_MASK) + env->tlb_table[MMU_USER_IDX][index].addend;   \
                ipact_struct *temp1 = NULL;                                                   \
                HASH_FIND(ipact_hash, ipact, &PPN, sizeof(target_ulong), temp1);              \
                /* Find an I-IPACT entry with right authority */                              \
                if(temp1 != NULL && temp1->kapp == Reg_kapp) {                                \
                    PA_log("[%d] Deleting 0x%" PRIx64 " from I-PACT", num, temp1->vpn);       \
                    /* Cloak the memory contents */                                           \
                    if (temp1->swap_bit == false) {                                           \
                        do_page_encrypt(temp1);                                               \
                        target_ulong val = is_sealed(temp, temp1->vpn);                       \
                        if (val != -1) {                                                      \
                            target_ulong pod_int_addr = val;                                  \
                            update_integrity_tag(env, pod_int_addr, temp1,                    \
                                                 COPY_TO_MEMORY, (uintptr_t) NULL,            \
                                                 !is_uninitialized(temp, temp1->vpn));        \
                        }                                                                     \
                    }                                                                         \
                    HASH_DELETE(ipact_hash, ipact, temp1);                                    \
                    free(temp1);                                                              \
                } else if (temp1 != NULL && temp1->kapp == Reg_kapp) {                        \
                    fprintf(stderr, "TLB miss while SHRINK_VA(%d) %" PRIx64, num, addr);      \
                }                                                                             \
                if (PG_SIZE > rem_size) {                                                     \
                    size +=  rem_size;                                                        \
                    rem_size = 0;                                                             \
                ...
                temp->qemu_delva_state = start + size;                                        \
            }                                                                                 \
})
    ...
    if (Reg_kapp) {
        ...
        if(temp) {
            assert(Reg_kapp == temp->kapp);
        }

        /* 231 is Exit */
        if (env->regs[R_EAX] == 231) {
            /* Here Reg_kapp is set implying we are in ring3
             * and are about to issue an exit OS system call.
             * We treat this equivalent of pod_exit() instruction
             * being issued from binary and do our safe termination
             * and then give control to OS to do the actual exit
             * system call
             */
            pod_exit(env);
        }
        /* 9 is Mmap */
        else if(env->regs[R_EAX] == 9) {
            temp->mmap_flag = 1;
            temp->qemu_delva_state = -1;
        }
        /* 11 is Munmap */
        else if(env->regs[R_EAX] == 11) {
            target_ulong munmap_addr = env->regs[R_EDI];
            target_ulong munmap_size = env->regs[R_ESI];
            if (temp->qemu_delva_state == -1) {
                PA_log("munmap() vaddr: 0x%" PRIx64 " size 0x%" PRIx64, munmap_addr, munmap_size);
                temp->qemu_delva_state = munmap_addr;
            } else {
                PA_log("[munmap] Doing delva from 0x%" PRIx64, temp->qemu_delva_state);
            }
            POD_DELVA(1, munmap_addr, munmap_addr + munmap_size);
            delete_interval(temp, munmap_addr, munmap_addr + munmap_size);

            /* [Optimization] When we do mmap and its corresponding
             * munmap, we will reuse the old integrity descriptor space
             * The mmap_flag stores sum of (addr + size) so we will
             * check if both addr and size match.
             */
            if (munmap_addr == (temp->mmap_flag - munmap_size)) {
                PA_log("Optimization (0x%" PRIx64 ", %" PRIx64 ")", munmap_addr, munmap_size);
                temp->mmap_VDTB -= (munmap_size / 4096) * 32;
            }

        /* 12 is brk */
        } else if (env->regs[R_EAX] == 12) {
            /* brk(0) is actually a query to find the begin
             * of the heap that is allocated to the process
             */
            if (temp->brk_zero_flag == 0) {
                temp->brk_zero_flag = 1;
                temp->qemu_delva_state = -1;
            } else {
                temp->brk_zero_flag = -1;
                if (temp->brk_end < env->regs[R_EDI]) {
                    temp->brk_flag = 1;
                }
                else {
                    if (temp->qemu_delva_state == -1) {
                        temp->qemu_delva_state = env->regs[R_EDI];
                        PA_log("brk() Decrease [0x%" PRIx64 "-> 0x%"
                            PRIx64 "]", env->regs[R_EDI], temp->brk_end);
                    } else {
                        PA_log("[brk] Doing delva from 0x%" PRIx64, temp->qemu_delva_state);
                    }
                    POD_DELVA(2, env->regs[R_EDI], temp->brk_end);
                    update_interval(temp, temp->brk_begin, temp->brk_end, env->regs[R_EDI]);
                    temp->brk_end = env->regs[R_EDI];
                }
            }
        }
    }
    ...
    if (env->hflags & HF_LMA_MASK) {
         ...
         * We will use it to check after syscall
         */
        if(Reg_kapp) {
            cr3_table_struct *temp = NULL;
            HASH_FIND(cr3_table_hash, cr3_table, &env->cr[3], sizeof(target_ulong), temp);
            if(temp) {
                assert(Reg_kapp == temp->kapp);
                temp->entry_point = env->regs[R_ECX];
                /* 15 is sys_rt_sigreturn()
                   The entry point will be different hence do not check */
                if (env->regs[R_EAX] == 15)
                    temp->entry_point = -1;
                PA_log("Syscall %" PRIu64 " from 0x%" PRIx64, env->regs[R_EAX], temp->entry_point);
                PA_log("EDI: %" PRIx64 " ESI: %" PRIx64 " EDX: %" PRIx64, env->regs[R_EDI], env->regs[R_ESI], env->regs[R_EDX]);
            }
        }
#endif
...
void helper_sysret(CPUX86State *env, int dflag)
{
    ...
    /* Secure Vectoring:
     * Ensure that the OS returned to the correct
     * entry point of the executable after system call
     */
    ...
    if(Reg_kapp) {
        ...
        if(temp) {
            assert(Reg_kapp == temp->kapp);
            /* Check the entry point expect after sys_rt_sigreturn() */
            if(temp->entry_point != -1 && temp->entry_point != env->regs[R_ECX]) {
                fprintf(stderr, "Syscall return is tampered (0x%" PRIx64 " != 0x%"
                        PRIx64 ")\n", temp->entry_point, env->regs[R_ECX]);
                /* CPU must do a safe termination here*/
                pod_exit(env);
            }
        }
        PA_log("Syscall ret to 0x%" PRIx64, env->regs[R_ECX]);
        temp->qemu_delva_state = -1;

        if (temp->mmap_flag == 1) {
            target_ulong mmap_size = env->regs[R_ESI];
            PA_log("mmap() vaddr: 0x%" PRIx64 " size: 0x%" PRIx64, env->regs[R_EAX], mmap_size);
            add_interval(temp, env->regs[R_EAX], env->regs[R_EAX] + mmap_size,
                         temp->mmap_VDTB, DATA_UNINITIALIZED);
            temp->mmap_VDTB += (mmap_size / 4096) * 32;
            temp->mmap_flag = env->regs[R_EAX] + mmap_size;
        }
        if (temp->brk_zero_flag == 1) {
            PA_log("brk() vaddr: 0x%" PRIx64, env->regs[R_EAX]);
            temp->brk_begin = env->regs[R_EAX];
            temp->brk_end   = env->regs[R_EAX];
            add_interval(temp, temp->brk_begin, temp->brk_end,
                         temp->brk_VDTB, DATA_UNINITIALIZED);
        }
        if (temp->brk_flag == 1) {
            PA_log("brk() Increase [0x%" PRIx64 "-> 0x%"
                   PRIx64 "]", temp->brk_end, env->regs[R_EDI]);
            update_interval(temp, temp->brk_begin, temp->brk_end, env->regs[R_EDI]);
            temp->brk_end = env->regs[R_EDI];
            temp->brk_flag = 0;
        }
