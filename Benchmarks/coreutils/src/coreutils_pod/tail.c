...
/* PodArch Marshalling for safe_read() */
static char __attribute__((section(".public_page"))) buffer[BUFSIZ];
            ...
            uintmax_t *read_pos)
{
  ...
  while (1)
    {
      n_read = safe_read (fd, buffer, BUFSIZ);
      memcpy(tmp->buffer, buffer, BUFSIZ);
  ...
  while (1)
    {
      ...
      if (len <= evbuf_off)
        {
          len = safe_read (wd, buffer, evlen);
          memcpy(evbuf, buffer, evlen);
          evbuf_off = 0;
