 ...
 * USAGE
 * -----
 * > gcc -o makepod makepod.c -lcrypto
 *
 * Converts the given binary to a pod-sealed one
 * and updates all the integrity tags
 *
 * Created by Deepak Kathayat, Viswesh Narayanan
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
static unsigned char* elf_write_binary_podintd(struct signelf_info *sinfo){
	...
	if(int_len!=sinfo->integrity_lend){
		printf("int_len!=sinfo->integrity_lend %x %x\n", int_len, sinfo->integrity_lend);
		goto out_free_shtable;
	}
...
static int encrypt_and_hash_elfc(struct signelf_info *sinfo){
	...
	for (i = 0; i < sinfo->ehdr.e_phnum, first_segment!=0; i++) {
		...
		while (sz_rem)  {
			...
			printf("CSTART 0x%x: *",(page_count*4096)+base);
...
static int encrypt_and_hash_elfd(struct signelf_info *sinfo)
{
	...
	for (i = 0; i < sinfo->ehdr.e_phnum, sec_segment !=0 ; i++) {
		...
		while (sz_rem) {
			...
			printf("DSTART 0x%x: *",(page_count*4096)+base);
			for(z=currentintsize; z<(currentintsize+TAGLEN);z++){
			 	printf("(%x)",(unsigned char)integritybuf[z]);
			}
			printf("*\n");
...
static int add_podkey_in_a_section(struct signelf_info *sinfo)
{
	...
	/* Add pod_id section */
	fclose(outfp);
	snprintf(command, 1024, "objcopy --add-section .pod_id=%s %s %s",
             tempkeysection_file, sinfo->in_file, sinfo->out_file);
	ret = system(command);
	if (ret == -1) {
...
static int sign_elf_executable(struct signelf_info *sinfo)
{
	...
	if(elf_write_binary_podintc(sinfo) == NULL) {
		fprintf(stdout, "Error while writing pod_intc into the elf\n");
		goto out;
	}

     if(elf_write_binary_podintd(sinfo) == NULL) {
	 	fprintf(stdout, "Error while writing pod_intd into the elf\n"); 
	 	goto out;
	 } 
