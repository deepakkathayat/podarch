...
/* PodArch Marshalling */
char __attribute__((section(".public_page"))) pub_area_fxstat[200];
struct stat __attribute__((section(".public_page"))) pub_st;
...
# endif
    {
      int i;
      for (i = 0; file[i] != '\0'; i++)
        pub_area_fxstat[i] = file[i];
      pub_area_fxstat[i] = '\0';

      res = INLINE_SYSCALL (newfstatat, 4, fd, pub_area_fxstat, &pub_st, flag);
      memcpy(st, &pub_st, sizeof(struct stat));
