...
/* PodArch Marshalling */
static char __attribute__((section(".public_page"))) pub_area_getlogin[50] = "/proc/self/loginuid\0";
     ...
     size_t namesize;
{
  ...
  int fd = open_not_cancel_2 (&pub_area_getlogin, O_RDONLY);
  if (fd == -1)
    return -1;
  ...
  char uidbuf[12];
  ssize_t n = TEMP_FAILURE_RETRY (read_not_cancel (fd, pub_area_getlogin,
						   sizeof (uidbuf)));
  close_not_cancel_no_status (fd);
  memcpy(&uidbuf, &pub_area_getlogin, sizeof(uidbuf));
