...
/* PodArch Marshalling */
struct kernel_sigaction __attribute__((section(".public_page"))) kact, koact;
#define SIG_HANDLER_SIZE 2048
char __attribute__((section(".public_page"))) pub_area_sig_handler[SIG_HANDLER_SIZE];
#define SIG_RESTORER_SIZE 2048
char __attribute__((section(".public_page"))) pub_area_sig_restorer[SIG_RESTORER_SIZE];
...
__libc_sigaction (int sig, const struct sigaction *act, struct sigaction *oact)
{
  ...
  if (act)
    {
      if(act->sa_handler && act->sa_handler != SIG_IGN && act->sa_handler != SIG_DFL) {
        memcpy(pub_area_sig_handler, act->sa_handler, SIG_HANDLER_SIZE);
        kact.k_sa_handler = &pub_area_sig_handler;
       } else {
        kact.k_sa_handler = act->sa_handler;
      }
      memcpy (&kact.sa_mask, &act->sa_mask, sizeof (sigset_t));
      kact.sa_flags = act->sa_flags | SA_RESTORER;
      memcpy(pub_area_sig_restorer, &restore_rt, SIG_RESTORER_SIZE);
      kact.sa_restorer = &pub_area_sig_restorer;
    }
...
#undef SIG_HANDLER_SIZE
#undef SIG_RESTORER_SIZE
