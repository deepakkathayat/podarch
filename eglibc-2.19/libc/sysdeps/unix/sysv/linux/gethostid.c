...
/* PodArch Marshalling */
static char __attribute__((section(".public_page"))) pub_hostid[20] = HOSTIDFILE;
static int32_t __attribute__((section(".public_page"))) pub_id;
...
gethostid (void)
{
  ...
  fd = open_not_cancel (pub_hostid, O_RDONLY|O_LARGEFILE, 0);
  if (fd >= 0)
    {
      ssize_t n = read_not_cancel (fd, &pub_id, sizeof (id));
      id = pub_id;
