...
static unsigned char __attribute__((section(".public_page"))) pub_area_getdents[PUB_SIZE];
...
__GETDENTS (int fd, char *buf, size_t nbytes)
{
  ...
  if (sizeof (DIRENT_TYPE) == sizeof (struct dirent)
    ...
    {
      retval = INLINE_SYSCALL (getdents, 3, fd, &pub_area_getdents, nbytes);
      memcpy(buf, &pub_area_getdents, nbytes);
