     ...
     int is32not64;
{
  ...
  if (__builtin_expect (fp->_flags2 & _IO_FLAGS2_NOTCANCEL, 0))
    fdesc = open_not_cancel ((char*) pub_area,
			     posix_mode | (is32not64 ? 0 : O_LARGEFILE), prot);
  else
    fdesc = open ((char*) pub_area, posix_mode | (is32not64 ? 0 : O_LARGEFILE), prot);
#else
  fdesc = open ((char*) pub_area, posix_mode, prot);
#endif
  if (fdesc < 0)
    return NULL;
  fp->_fileno = fdesc;
     ...
     _IO_FILE *fp;
{
  ...
  count = _IO_SYSREAD (fp, (char*) pub_area,
		       fp->_IO_buf_end - fp->_IO_buf_base);
  if (count > 0) {
    memcpy(fp->_IO_buf_base, &pub_area, count);
  }
  if (count <= 0)
    {
      if (count == 0)
  ...
  while (to_do > 0)
    {
      _IO_ssize_t i;
      for (i = 0; i < to_do; i++)
        pub_area[i] = *((char*)(data + i));

      _IO_ssize_t count = (__builtin_expect (f->_flags2
					     & _IO_FLAGS2_NOTCANCEL, 0)
			   ? write_not_cancel (f->_fileno, pub_area, to_do)
			   : write (f->_fileno, pub_area, to_do));
      if (count < 0)
	{
	  f->_flags |= _IO_ERR_SEEN;
	  break;
     ...
     _IO_size_t n;
{
  ...
  while (want > 0)
      ...
      else
	{
	  ...
	  count = _IO_SYSREAD (fp, &pub_area, count);
      memcpy(s, &pub_area, count);
