...
static struct utmp __attribute__((section(".public_page"))) last_entry;
...
/* PodArch Marshalling */
char __attribute__((section(".public_page"))) pub_area_utmp[200];
...
# define O_flags O_LARGEFILE
      ...
      for (i = 0; file_name[i] != '\0'; i++)
        pub_area_utmp[i] = file_name[i];
      pub_area_utmp[i] = '\0';

      file_fd = open_not_cancel_2 ((char*) pub_area_utmp, O_RDONLY | O_flags);
      if (file_fd == -1)
	return 0;
